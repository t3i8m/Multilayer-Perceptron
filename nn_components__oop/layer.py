...
class Layer(object):
    ...
    def get_biases(self)->np.array:
        ...
        biases = [n.get_bias() for n in self.neurons]
        return np.array(biases).reshape(-1, 1)
    ...
    def initialize_weights_biases(self)->None:
        ...
        self.neurons = [Neuron(self.prev_layer_size, float(bias)) for _, bias in zip(range(self.size), np.random.randn(self.size, 1))]
        return
    ...
    def set_gradients_bias(self, gradients:np.array)->None:
        ...
        for neuron, grad in zip(self.neurons, gradients):
            neuron.add_gradients_biases(grad)
        return 
